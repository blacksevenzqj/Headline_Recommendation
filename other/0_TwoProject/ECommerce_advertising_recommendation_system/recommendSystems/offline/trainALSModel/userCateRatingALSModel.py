...
class UserCateRatingALSModel(object):
    ...
    HDFS_PATH = "hdfs://localhost:9000/project1-ad-rs/preprocessing-datasets/cate_count.csv"
    ...
    def initial(self):
        ...
        self.user_cate_count_df = self.get_user_cate_count_df()
    ...
    def create_user_cate_rating_df(self):
        # 用户对商品类别的打分数据
        # map返回的结果是rdd类型，需要调用toDF方法转换为Dataframe
        self.user_cate_rating_df = self.user_cate_count_df.rdd.map(self.process_row).toDF(["userId", "cateId", "rating"])

    def get_user_cate_rating_matrix(self):
        # 可通过该方法获得 user-cate-matrix
        # 但由于cateId字段过多，这里运算量比很大，机器内存要求很高才能执行，否则无法完成任务
        # 请谨慎使用

        # 但好在我们训练ALS模型时，不需要转换为user-cate-matrix，所以这里可以不用运行
        self.user_cate_rating_df.groupBy("userId").povit("cateId").min("rating")

    def train_model_and_save(self, user_cate_rating_df):
        ...
        # 此处训练时间较长
        model = als.fit(user_cate_rating_df)
        # 保存模型
